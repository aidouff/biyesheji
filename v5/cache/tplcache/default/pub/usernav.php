...
<?php require_once ("C:/phpstudy/WWW/taglib/usernav.php");$usernav_tag = new Taglib_Usernav();if (method_exists($usernav_tag, 'topkind')) {$data = $usernav_tag->topkind(array('action'=>'topkind','row'=>'6',));}?>
 <?php if(!empty($data)) { ?>
        ...
        <?php $n=1; if(is_array($data)) { foreach($data as $nav) { ?>
            <div class="gl-list-tabbox">
                <h3>
                    ...
                    <p>
                        <?php require_once ("C:/phpstudy/WWW/taglib/usernav.php");$usernav_tag = new Taglib_Usernav();if (method_exists($usernav_tag, 'childnav')) {$childnav = $usernav_tag->childnav(array('action'=>'childnav','parentid'=>$nav['id'],'row'=>'5','return'=>'childnav',));}?>
                ...
                <div class="tabcon-item">
                    <div class="item-list">
                        <?php require_once ("C:/phpstudy/WWW/taglib/usernav.php");$usernav_tag = new Taglib_Usernav();if (method_exists($usernav_tag, 'childnav')) {$childnav2 = $usernav_tag->childnav(array('action'=>'childnav','parentid'=>$nav['id'],'row'=>'10','return'=>'childnav2',));}?>
                          <?php $n=1; if(is_array($childnav2)) { foreach($childnav2 as $r2) { ?>
                              <dl>
                                <dt><a href="<?php echo $r2['url'];?>" target="_blank"><?php echo $r2['kindname'];?></a></dt>
                                <dd>
                                    <?php require_once ("C:/phpstudy/WWW/taglib/usernav.php");$usernav_tag = new Taglib_Usernav();if (method_exists($usernav_tag, 'childnav')) {$childnav3 = $usernav_tag->childnav(array('action'=>'childnav','parentid'=>$r2['id'],'return'=>'childnav3','row'=>'20',));}?>
                    ...
                    <div class="ad-box">
                        <?php require_once ("C:/phpstudy/WWW/taglib/ad.php");$ad_tag = new Taglib_Ad();if (method_exists($ad_tag, 'sortad')) {$pluginad = $ad_tag->sortad(array('action'=>'sortad','index'=>$k,'pc'=>'1','adname'=>'Header_Usernav_1,Header_Usernav_2,Header_Usernav_3,Header_Usernav_4,Header_Usernav_5,Header_Usernav_6','return'=>'pluginad',));}?>
